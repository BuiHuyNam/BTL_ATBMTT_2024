	...
	<div class="main-wrapper">
		<section class="cta-section theme-bg-light py-5">
			<div class="container text-center">
				<?php
				$query = "SELECT * FROM tbl_danhMuc WHERE id_danhMuc = '$_GET[id]'";
				$sql = mysqli_query($database, $query);
				if (mysqli_num_rows($sql) > 0) {
					while ($row = mysqli_fetch_array($sql)) {
						echo "
								<h2 class='heading'>Thông tin về lỗ hỏng bảo mật " . $row['tenDanhMuc'] . " </h2>
								";
					}
				}
				...
				<!-- <form class="signup-form form-inline justify-content-center pt-3">
					<div class="form-group">
						...
						<input type="email" id="semail" name="semail1" class="form-control mr-md-1 semail" placeholder="Tra cứu...">
					</div>
		...
		<section class="blog-list px-3 py-5 p-md-5">
			<div class="container">
				<?php
				$query = "SELECT * FROM tbl_noiDung WHERE id_danhMuc = '$_GET[id]'";
				$sql = mysqli_query($database, $query);
				if (!$sql) {
					echo " Truy van that bai";
				}
